...
app = FastAPI(title="TinyD API", version="1.1")
...
if _allowed_origins:
    app.add_middleware(CORSMiddleware, allow_origins=_allowed_origins, allow_credentials=True,
                       allow_methods=["GET"], allow_headers=["X-TinyD-Tenant-ID", "Authorization", "Content-Type"])


def tenant(value: str | None) -> str:
    if not value or len(value) > 128 or value.strip() != value:
        raise HTTPException(400, "X-TinyD-Tenant-ID is required")
    return value
...
def assurance_summary(x_tinyd_tenant_id: str | None = Header(default=None)):
    t = tenant(x_tinyd_tenant_id)
    rows = query("""
        SELECT count(*)::bigint AS evidence_count,
               count(DISTINCT execution_id)::bigint AS execution_count,
               count(*) FILTER (WHERE artifact_digest IS NOT NULL)::bigint AS artifact_count,
               count(*) FILTER (WHERE policy_version IS NOT NULL)::bigint AS policy_bound_count
        FROM evidence_ledger WHERE tenant_id = %s
    """, (t,))
    ...
    return {**rows[0], **verified[0], **failed[0], "tenant_id": t}


@app.get("/api/v1/executions")
def executions(x_tinyd_tenant_id: str | None = Header(default=None), limit: int = Query(50, ge=1, le=200)):
    t = tenant(x_tinyd_tenant_id)
    return query("""
        SELECT execution_id, min(created_at) AS first_seen, max(created_at) AS last_seen,
               count(*)::bigint AS event_count, max(event_type) AS last_event_type
        FROM evidence_ledger WHERE tenant_id=%s GROUP BY execution_id ORDER BY last_seen DESC LIMIT %s
    """, (t, limit))


@app.get("/api/v1/evidence")
def evidence(x_tinyd_tenant_id: str | None = Header(default=None), execution_id: str | None = None,
             limit: int = Query(100, ge=1, le=500)):
    t = tenant(x_tinyd_tenant_id)
    if execution_id:
        return query("SELECT * FROM evidence_ledger WHERE tenant_id=%s AND execution_id=%s ORDER BY sequence DESC LIMIT %s", (t, execution_id, limit))
    return query("SELECT * FROM evidence_ledger WHERE tenant_id=%s ORDER BY sequence DESC LIMIT %s", (t, limit))


@app.get("/api/v1/evidence/{evidence_id}")
def evidence_by_id(evidence_id: str, x_tinyd_tenant_id: str | None = Header(default=None)):
    t = tenant(x_tinyd_tenant_id)
    rows = query("SELECT * FROM evidence_ledger WHERE tenant_id=%s AND evidence_id=%s", (t, evidence_id))
    if not rows: raise HTTPException(404, "evidence not found")
    return rows[0]


@app.get("/api/v1/provenance/{subject_id}")
def provenance(subject_id: str, x_tinyd_tenant_id: str | None = Header(default=None)):
    t = tenant(x_tinyd_tenant_id)
    nodes = query("SELECT * FROM provenance_nodes WHERE tenant_id=%s AND node_id=%s", (t, subject_id))
...
def authorization(execution_id: str, x_tinyd_tenant_id: str | None = Header(default=None)):
    t = tenant(x_tinyd_tenant_id)
    return query("""
        SELECT DISTINCT policy_id, policy_version, policy_digest, event_id, actor, capability, operation, created_at
        FROM evidence_ledger WHERE tenant_id=%s AND execution_id=%s AND policy_version IS NOT NULL ORDER BY created_at DESC
    """, (t, execution_id))


@app.get("/api/v1/artifacts/{digest}")
def artifact(digest: str, x_tinyd_tenant_id: str | None = Header(default=None)):
    t = tenant(x_tinyd_tenant_id)
...
def replay(execution_id: str, x_tinyd_tenant_id: str | None = Header(default=None)):
    t = tenant(x_tinyd_tenant_id)
...
def policies(x_tinyd_tenant_id: str | None = Header(default=None), limit: int = Query(100, ge=1, le=500)):
    t = tenant(x_tinyd_tenant_id)
...
def audit(x_tinyd_tenant_id: str | None = Header(default=None), limit: int = Query(100, ge=1, le=500)):
    t = tenant(x_tinyd_tenant_id)
