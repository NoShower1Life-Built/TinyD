...
class IndependentEvidenceVerifier:
    ...
    def verify(self, event: EventEnvelope) -> VerificationResult:
        ...
        body = {"subject_digest": event.integrity_digest, "result": "VERIFIED" if ok else "FAILED",
                "verifier_id": self.verifier_id, "verifier_version": self.verifier_version, "reason": reason}
        verification_digest = hashlib.sha256(json.dumps(body, sort_keys=True, separators=(",", ":")).encode()).hexdigest()
        return VerificationResult(hashlib.sha256((event.event_id + self.verifier_version).encode()).hexdigest(),
                                  event.integrity_digest, body["result"], self.verifier_id,
                                  self.verifier_version, verification_digest, reason)
...
class DeterministicReplayVerifier:
    ...
    def replay(self, execution_id: str, events: Sequence[EventEnvelope]) -> VerificationResult:
        ...
        valid = all(e.verify_integrity() for e in selected)
        previous = None
        for event in selected:
            if event.parent_event_id and event.parent_event_id != previous:
                valid = False
                break
        ...
        return VerificationResult(hashlib.sha256((execution_id + self.verifier_version).encode()).hexdigest(),
                                  replay_digest, result, self.verifier_id, self.verifier_version,
                                  verification_digest, "deterministic replay comparison")
