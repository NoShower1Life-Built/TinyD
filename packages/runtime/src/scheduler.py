...
class DurableScheduler:
    ...
    authoritative event history and is accessed by the runtime through
    EventJournal rather than by this scheduler.
    """
    ...
    def complete(self, work_id: UUID, worker_id: str) -> bool:
        if not worker_id:
            raise ValueError("worker_id must not be empty")
        with self._connection.cursor() as cursor:
            cursor.execute(
                ...
                WHERE work_id = %s
                  ...
                  AND lease_owner = %s
                """,
                (datetime.now(timezone.utc), str(work_id), worker_id),
            )
        ...
        with self._connection.cursor() as cursor:
            cursor.execute(
                ...
                WHERE work_id = %s
                  ...
                  AND lease_owner = %s
                """,
                (max_attempts, max_attempts, when, error, str(work_id), worker_id),
            )
            updated = cursor.rowcount
            self._connection.commit()
        return updated == 1
        ...
        with self._connection.cursor() as cursor:
            cursor.execute(
                ...
                SET status = 'PENDING',
                    ...
                    lease_expires_at = NULL
                FROM expired
                WHERE w.work_id = expired.work_id
                """,
...
def _row_to_work_item(row: tuple[Any, ...]) -> WorkItem:
    ...
    return WorkItem(
        ...
        tenant_id=row[1], aggregate_id=row[2], run_id=row[3], event_id=row[4],
        status=row[5], attempt_count=int(row[6]), available_at=_utc(row[7]),
        lease_owner=row[8], lease_expires_at=_utc(row[9]) if row[9] is not None else None,
        last_error=row[10], created_at=_utc(row[11]),
        completed_at=_utc(row[12]) if row[12] is not None else None,
    )
