...
CREATE TABLE IF NOT EXISTS evidence_ledger (
    ...
    PRIMARY KEY (tenant_id, sequence), UNIQUE (tenant_id, evidence_id), UNIQUE (tenant_id, idempotency_key),
    UNIQUE (tenant_id, event_id), UNIQUE (tenant_id, record_digest)
);

CREATE TABLE IF NOT EXISTS policy_versions (
    tenant_id TEXT NOT NULL, policy_id TEXT NOT NULL, version TEXT NOT NULL, policy_digest CHAR(64) NOT NULL,
    policy_json JSONB NOT NULL, created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
    PRIMARY KEY (tenant_id, policy_id, version), UNIQUE (tenant_id, policy_id, policy_digest)
);
...
CREATE TABLE IF NOT EXISTS provenance_nodes (
    node_id TEXT PRIMARY KEY, tenant_id TEXT NOT NULL, node_type TEXT NOT NULL, subject_digest CHAR(64),
    created_at TIMESTAMPTZ NOT NULL DEFAULT now(), UNIQUE (tenant_id, node_id)
);
CREATE TABLE IF NOT EXISTS provenance_edges (
    source_id TEXT NOT NULL, relationship TEXT NOT NULL, target_id TEXT NOT NULL, tenant_id TEXT NOT NULL,
    created_at TIMESTAMPTZ NOT NULL DEFAULT now(), PRIMARY KEY (tenant_id, source_id, relationship, target_id),
    FOREIGN KEY (tenant_id, source_id) REFERENCES provenance_nodes(tenant_id, node_id),
    FOREIGN KEY (tenant_id, target_id) REFERENCES provenance_nodes(tenant_id, node_id)
);

CREATE TABLE IF NOT EXISTS ownership_records (
    tenant_id TEXT NOT NULL, subject_id TEXT NOT NULL, relationship TEXT NOT NULL, target_id TEXT NOT NULL,
    evidence_digest CHAR(64) NOT NULL, created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
    PRIMARY KEY (tenant_id, subject_id, relationship, target_id)
);
